"""
sim_lane_keeping_real_animate.py — 真实赛道车道保持动画（流畅版）

预计算所有帧数据 + 赛道总览小窗，丝滑播放。

用法:
  python pipeline/sim_lane_keeping_real_animate.py                # 交互播放
  python pipeline/sim_lane_keeping_real_animate.py --save output/animation.gif
"""
...
def precompute_car_polys(n_frames, skip, car_x, car_y, psi_car, steer):
    ...
    for i in range(n_frames):
        ...
        for ax, ay, angle in [(rwx, rwy, ps), (fwx, fwy, ps + st)]:
            ...
            parts.append((wh @ Rw2.T + [ax, ay], "#1d3557", "#000", 0.8))
        frames.append(parts)
    return frames
...
def run():
    ...
    ap.add_argument("--file", default="output/sim_lane_keeping_real.txt")
    ap.add_argument("--save", default=None, help="Save GIF")
    ap.add_argument("--interval", type=int, default=25, help="ms per frame")
    ap.add_argument("--skip", type=int, default=2, help="Every N steps = 1 frame")
    ap.add_argument("--speed", type=float, default=1.0, help="Playback speed multiplier")
    ap.add_argument("--full", action="store_true", help="Fixed global view")
    args = ap.parse_args()

    print("Loading simulation data ...")
    header, data = load_log(args.file)
    ...
    traj, bounds = build_traj_and_bounds()
    ...
    # Real-time sync: each frame advances skip*dt sim seconds
    ...
    # ---- Pre-compute ALL per-frame data ----
    print(f"Pre-computing {nf} frames ...")
    ...
    trail_x_frames = [car_x[:idx+1] for idx in indices]
    ...
    eps   = [ep_deg[idx] for idx in indices]
    strs  = [st_deg[idx] for idx in indices]
    steps = [idx * Vx * dt for idx in indices]
    ...
    print(f"  Done. {nf} frames ready, track={traj.total_len:.0f}m, "
          f"interval={interval}ms (speed={speedup:.1f}x).")

    # ======================== Figure ========================
    fig = plt.figure(figsize=(18, 10))
    fig.suptitle("Lane Keeping — Bicycle Model on Real Track", fontsize=14)
    ...
    ax_map.plot(traj.points[:,0], traj.points[:,1], "b--", lw=0.6, alpha=0.3)
    ...
    # ---- Overview inset (top-right of main map) ----
    ...
    # ---- Right panel: 3 error/time-series subplots ----
    ...
    def update(frame):
        ...
        cx, cy = car_x[indices[idx]], car_y[indices[idx]]
        if not args.full:
            ax_map.set_xlim(cx - zoom, cx + zoom)
            ax_map.set_ylim(cy - zoom, cy + zoom)

        info_txt.set_text(
            f"t={times[idx]:.1f}s  s={steps[idx]:.0f}/{traj.total_len:.0f}m\n"
            f"e_y={eys[idx]:+.3f}m  e_psi={eps[idx]:+.1f}deg  steer={strs[idx]:+.1f}deg")

        # ---- Overview update ----
        ov_trail.set_data(car_x[:indices[idx]+1], car_y[:indices[idx]+1])
        ov_dot.set_data([cx], [cy])

        # Error plots
        n_data = indices[idx] + 1
        ey_line.set_data(t_arr[:n_data], e_y[:n_data])
        ey_cur.set_data([times[idx]], [eys[idx]])
        ep_line.set_data(t_arr[:n_data], ep_deg[:n_data])
        ep_cur.set_data([times[idx]], [eps[idx]])
    ...
    ani = FuncAnimation(fig, update, frames=nf,
