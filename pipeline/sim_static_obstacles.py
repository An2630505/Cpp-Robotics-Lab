"""
sim_static_obstacles.py — 静态障碍物场景仿真

管线: path2.png → map_parser → centerline → circuit
      → occupancy grid → static_obstacles → HA*
      → SafeCorridor → BSpline → MPC → 仿真

用法: python pipeline/sim_static_obstacles.py
"""
...
# B-Spline 参数
BSPLINE_DEGREE       = 3
BSPLINE_NUM_CTRL     = 100     # 更多控制点 → 紧贴 HA* 路径
BSPLINE_RESAMPLE     = 0.5     # 重采样间距 (m)

# 障碍物配置文件
OBSTACLES_JSON = os.path.join(_project_root, "config", "obstacles.json")
...
def build_occupancy_grid(outer, holes, cell_size, safety_margin):
    ...
    Returns: grid (list[list[int]]), meta dict
      grid: 0=自由, 1=障碍物
    """
...
def load_obstacles(json_path: str):
    """从 JSON 配置文件加载静态障碍物.

    如果文件不存在，返回空的 ObstacleLayer (不含障碍物).
    """
    from pipeline.static_obstacles import ObstacleLayer

    obs_layer = ObstacleLayer()
    if os.path.exists(json_path):
        n = obs_layer.add_from_json(json_path)
        print(f"  加载 {n} 个障碍物 from {json_path}")
    else:
        print(f"  ⚠ 障碍物配置文件不存在: {json_path}，跳过")
    return obs_layer
...
def run():
    ...
    # Step 4: 加载静态障碍物
    print(f"[4/8] 加载静态障碍物: {OBSTACLES_JSON}")
    obs_layer = load_obstacles(OBSTACLES_JSON)

    # Step 5: 构建占用栅格 (障碍物合并到 holes → 栅格统一处理)
    print("[5/8] 构建占用栅格 ...")
    grid_holes = list(map_holes)
    if len(obs_layer) > 0:
        grid_holes.extend(obs_layer.to_polygons())
    grid, grid_meta = build_occupancy_grid(outer, grid_holes, CELL_SIZE, SAFETY_MARGIN)
    ...
    # Step 6: 生成 Gate + HA* 规划
    print("[6/8] Hybrid A* 规划 ...")
    ...
    # Step 7: Safe Corridor + B-Spline
    print("[7/8] SafeCorridor + B-Spline 平滑 ...")
    ...
    np.save("output/sim_static_obstacles_traj.npy", pts)
    np.save("output/sim_static_obstacles_outer.npy", outer)
    for i, h in enumerate(map_holes):
        np.save(f"output/sim_static_obstacles_hole_{i}.npy", h)
    if corridors:
        corr_arr = np.array([[(s.left.x, s.left.y),
                               (s.center.x, s.center.y),
                               (s.right.x, s.right.y)] for s in corridors])
        np.save("output/sim_static_obstacles_corridors.npy", corr_arr)

    # Step 8: MPC 仿真
    print(f"\n[8/8] MPC 仿真 ({VX:.0f} m/s, dt={DT}s) ...")
    ...
    print(f"  Simulating {N_STEPS} steps = {N_STEPS*DT:.1f}s "
          f"= {N_STEPS*VX*DT:.1f}m")
    ...
    # 保存文本日志
    out_txt = "output/sim_static_obstacles.txt"
    with open(out_txt, "w") as f:
        f.write(f"# REF: type=static_obstacles len={traj.total_len:.1f}m "
                f"dt={DT} Vx={VX}\n")
    ...
    # 可视化
    visualize(log, traj, raw_path, smoothed, outer, map_holes, obs_layer, grid_meta,
              corridors)
...
def visualize(log, traj, raw_path, smoothed, outer, holes, obs_layer, grid_meta,
              corridors=None):
    arr = np.array(log)
    N = len(arr)
    ts  = arr[:, 1]
    ...
    ax.set_title("Track, Obstacles & Optimized Path")
    ...
    if corridors is not None and len(corridors) > 0:
        lx = [c.left.x for c in corridors]
        ly = [c.left.y for c in corridors]
        rx = [c.right.x for c in corridors]
        ry = [c.right.y for c in corridors]
    ...
    # 静态障碍物 (在走廊之后绘制, 覆盖侵入障碍物的走廊)
    if len(obs_layer) > 0:
        obs_polys = obs_layer.to_polygons()
        for i, poly in enumerate(obs_polys):
            ax.fill(poly[:,0], poly[:,1], fc="red", ec="darkred",
                    lw=1.5, alpha=0.5,
                    label=f"Obstacle {i}" if i == 0 else "")
    ...
    out_png = "output/sim_static_obstacles.png"
