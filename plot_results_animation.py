...
with open('output/output.txt', 'r') as f:
    for line in f:
        ...
        if len(parts) == 9:
            step = int(parts[0])
            self_y_0 = float(parts[1])
            self_y_1 = float(parts[2])
            self_y_2 = float(parts[3])
            self_y_3 = float(parts[4])
...
fig.suptitle('Control Algorithm Results: Trajectory Animation', fontsize=16)

# 初始化线条
line_self, = ax.plot([], [], 'b-', linewidth=2, label='Self Position (Filtered)')
line_obj, = ax.plot([], [], 'r--', linewidth=2, label='Target Position (Estimated)')
point_self, = ax.plot([], [], 'bo', markersize=8)
point_obj, = ax.plot([], [], 'rs', markersize=8)

# 设置坐标轴范围 (可以根据实际数据动态调整，这里先预设一个大致范围或稍后自动调整)
# 为了动画流畅，通常先设定好固定的 axis limit，或者在 init 中设置
min_x = min(self_y_0.min(), obj_y_0.min()) - 1
max_x = max(self_y_0.max(), obj_y_0.max()) + 1
min_y = min(self_y_1.min(), obj_y_1.min()) - 1
max_y = max(self_y_1.max(), obj_y_1.max()) + 1

ax.set_xlim(min_x, max_x)
ax.set_ylim(min_y, max_y)
ax.set_xlabel('X Position', fontsize=12)
ax.set_ylabel('Y Position', fontsize=12)
ax.set_title('Trajectory: Self vs Target', fontsize=14)
...
def init():
    ...
    point_obj.set_data([], [])
    time_text.set_text('')
    return line_self, line_obj, point_self, point_obj, time_text

def update(frame):
    # frame 是当前帧的索引
    # 绘制从 0 到 frame 的数据
    current_self_x = self_y_0[:frame+1]
    current_self_y = self_y_1[:frame+1]
    current_obj_x = obj_y_0[:frame+1]
    current_obj_y = obj_y_1[:frame+1]
    
    line_self.set_data(current_self_x, current_self_y)
    line_obj.set_data(current_obj_x, current_obj_y)
    
    # 更新当前点的位置
    if frame < len(self_y_0):
        point_self.set_data([self_y_0[frame]], [self_y_1[frame]])
