...
Plant_car::Plant_car(Eigen::MatrixXd A, Eigen::MatrixXd B1, Eigen::MatrixXd B2, Eigen::MatrixXd C, Eigen::MatrixXd D)
{
    this->t = 0.0;
    this->nx = A.rows();
    this->ny = C.rows();
...
Eigen::VectorXd Plant_car::Init(Eigen::VectorXd x0, Eigen::VectorXd u0)
{
    this->t = 0.0;
    this->x0 = x0;
    this->u0 = u0;
...
Eigen::VectorXd Plant_car::step( float dt ,  float w ,const Eigen::VectorXd& u)
{

    Eigen::VectorXd dot_x(this->nx);

    // 状态更新
    dot_x = this->A * this->x + this->B1 * u + this->B2 * w ;
...
float Plant_car::getTime()
{
    return this->t;
}

Eigen::VectorXd Plant_car::sampleGaussian(const Eigen::MatrixXd &cov) 
{
    int n = cov.rows();
    Eigen::VectorXd z(n);
...
Eigen::VectorXd Plant_car::generateFrictionDisturbance(
